...
- **정규식 기반 탐지의 한계**: 패턴 매칭이므로 오탐(유사 숫자 마스킹)·미탐(변형된 PII 누락) 가능성이 있습니다. 체크디지트 검증(`verify-checkdigit`)은 선택 제공하되 **기본 비활성**입니다 — 2020년 10월 이후 발급·변경된 주민등록번호는 검증번호(체크섬)가 폐지되어, 검증을 강제하면 정상 PII가 마스킹되지 않고 누출될 수 있기 때문입니다(가드레일은 보수적 마스킹이 안전).
- **ACCOUNT 유형 과탐**: 계좌번호는 보편적 체크섬이 없어 패턴이 느슨합니다. 날짜·전화·카드 등의 오탐을 막는 포맷 배제 필터를 항상 적용하나, 잔여 과탐 가능성은 있습니다.
